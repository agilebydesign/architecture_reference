...
import { BotBehavior, type BotBehaviorHydrateData, type IBehaviorConfig, type IBaseActionConfig, type NavigationResult, type ExecutionSetting } from "../bot_behavior.js";
...
export class BotBehaviorClient extends BotBehavior {
  ...
  /** Update the behavior tree DOM to reflect current selection (active classes only, not expanded). */
  updateTreeDom(): void {
    ...
    behaviorItems.forEach((li) => {
      ...
      // Update expand icon to reflect current expanded state
  ...
  /** Toggle expand/collapse of a behavior's action list. */
  ...
  override navigateToBehavior(name: string): void {
    ...
    this._updateDom();
    this.updateTreeDom();
    this.syncToServer("botBehavior.navigateToBehavior", name);
  }
  ...
  override navigateToAction(name: string): void {
    ...
    this._updateDom();
    this.updateTreeDom();
    this.syncToServer("botBehavior.navigateToAction", name);
  }
  ...
  override next(): NavigationResult {
    ...
    this._updateDom();
    this.updateTreeDom();
    this.syncToServer("botBehavior.next");
    return result;
  }
  ...
  override back(): NavigationResult {
    ...
    this._updateDom();
    this.updateTreeDom();
    this.syncToServer("botBehavior.back");
    return result;
  }
  ...
  override closeCurrent(): NavigationResult {
    ...
    this._updateDom();
    this.updateTreeDom();
    this.syncToServer("botBehavior.closeCurrent");
    return result;
  }
  ...
  override setExecutionSetting(key: string, value: ExecutionSetting): void {
    ...
    this.vscode.postMessage({ command: "botBehavior.setExecutionSetting", key, value });
  }

  /** Update the execution setting button active state for a specific target key. */
  ...
  /** Render the behavior tree HTML from a tree data structure. */
  renderTree(treeData: BehaviorTreeEntry[], currentBehavior: string, currentAction: string): void {
    if (!this.behaviorTreeEl) return;

    // Preserve currently expanded behaviors across re-renders
  ...
  /** Attach click handlers to the behavior tree elements. */
  attachTreeListeners(): void {
    if (!this.behaviorTreeEl) return;

    // Expand icon click → toggle expand/collapse (does NOT navigate)
    ...
    // Behavior name click → select behavior (navigate, does NOT toggle expand)
    ...
    // Action item click → navigate to action
    this.behaviorTreeEl.querySelectorAll(".action-item").forEach((item) => {
      item.addEventListener("click", (e) => {
        ...
        if (behaviorName && actionName) {
          // Navigate to the behavior first if not current
  ...
  override hydrate(data: BotBehaviorHydrateData & { behaviorTree?: BehaviorTreeEntry[] }): void {
    // If full init payload, load behaviors and actions into local domain first
...
export function initBotBehaviorClient(vscode: VsCodeApi): BotBehaviorClient {
  ...
  if (nextBtn) nextBtn.addEventListener("click", () => botBehavior.next());
  if (backBtn) backBtn.addEventListener("click", () => botBehavior.back());
  ...
  // Attach tree listeners to server-rendered HTML on initial load
  botBehavior.attachTreeListeners();

  // Request full init state from server (behaviors, actions, configs)
  vscode.postMessage({ command: "botBehavior.requestInit" });
